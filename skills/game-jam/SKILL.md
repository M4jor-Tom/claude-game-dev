...
## In ontology-first repos

Jam builds are **exempt** from the ontology-first rule: the timebox wins, ship on a separate
branch or repo. Jam code is pre-ontology and never merges into an ontology-first repo — if the
game lives on after the jam, start it with `game-from-ontology` and model what the jam taught
you; don't inherit the jam code as the foundation.
