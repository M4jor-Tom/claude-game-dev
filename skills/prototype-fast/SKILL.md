...
Spike code is **pre-ontology and never merges**. It exists to answer "is it fun?" — nothing
else. On a *keep* verdict, the mechanic gets modeled in `ontology/` first (see
`game-from-ontology`), then rebuilt by consuming the ontology; the spike itself is deleted.
