...
0. **Sync the ontology** (ontology-first repos — an `ontology/` directory exists, or
   `game-from-ontology` governs the repo). Before routing, land the requested change in the
   ontology: new or modified classes, relations, constraints, generators, or instances go to
   `ontology/` *first*, and the routed skills then implement by **consuming** the updated
   ontology — code and data derive from it, never drift ahead of it. Skip this step only for
   tasks that don't touch the domain (debugging, build/export, tooling) and for
   `prototype-fast`/`game-jam` work, which is pre-ontology by exemption.
